...
class Attendance(models.Model):
    ...
    attendance_name = models.CharField(null=True, max_length=255)
    ...
    def __str__(self):
        return f"{self.student} - {self.attendance} - {self.title} - {self.date} - {self.start_time} - {self.end_time} - {self.status} - {self.description} - {self.teacher_email} - {self.recurring_weeks} - {self.absence_reason} - {self.attendance_name}"
